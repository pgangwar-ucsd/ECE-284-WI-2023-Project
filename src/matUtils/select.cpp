    ...
    for (auto s: dfs) {
        ...
        if (canns.size() > 0) {
            if (canns.size() > 1 || canns[0] != "") {
                ...
                for (auto c: canns) {
                    if (c == clade_name) {
                        //std::cout << "\nNode: " << s->identifier.c_str() << std::endl;
                        ...
                        /////////////////////TEST
                        //for (auto node: csamples) {
                        //    if (node == "England/ALDP-C3A168/2020|OD909373.1|2020-12-08") {
                        //        std::cout << std::endl << node << std::endl << std::endl;
                        //        std::vector<Mutation_Annotated_Tree::Node*> ancestors = T->rsearch(node, true);
                        //        for (auto anc: ancestors) {
                        //            if ((anc->identifier == "node_522") || (anc->identifier == "node_516")) {
                        //                std::cout << anc->identifier.c_str(); 
                        //                for (auto mut: anc->mutations)
                        //                    std::cout << ", Mut_pos: " << mut.position << ", Ref: " << mut.ref_nuc << ", Mut: " << mut.mut_nuc;
                        //            std::cout << "\n";
                        //            }
                        //        }
                        //    }
